...
template<uint X, uint Y> // HLSL 2021 template function
struct Palette
{
	uint index : 2; // HLSL 2021 bitfield

	template<typename TEX> // HLSL 2021 template function
	float4 operator[](TEX tex) // HLSL 2021 operator overloading
	{
		return tex[index].Load(int3(X, Y, 0));
	}
};
...
float4 main(Input input) : SV_Target {
	...
	if (RootParamOffset < 8) {
		Palette<0, 0> palette = { RootParamOffset };
		color = palette[ColorMap];
	} else {
		uint index = ((uint)(input.position.x) + (uint)(input.position.y)) % 8;
		Palette<0, 0> palette = { NonUniformResourceIndex(index) };
		color = palette[ColorMap];
	}

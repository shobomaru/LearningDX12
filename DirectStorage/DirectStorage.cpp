...
class D3D
{
	...
	HANDLE mDStorageSignalHandle = INVALID_HANDLE_VALUE;
	ComPtr<ID3D12Fence> mDStorageFence;
...
public:
	~D3D()
	{
		...
		// Check DirectStorage failure
		WaitForSingleObject(mDStorageSignalHandle, 3000);
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		static const auto* filePath = L"tempfile.bin";
		{
		...
		// I found this necessary if dstorage.dll is marked as delayed load
		LoadLibrary(L"dstoragecore.dll");

		mDStorageSignalHandle = CreateEvent(nullptr, FALSE, FALSE, nullptr);
		CHK(mDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&mDStorageFence)));

		DSTORAGE_CONFIGURATION dstorageConfig = {};
		dstorageConfig.DisableTelemetry = TRUE; // I'm shy
		CHK(DStorageSetConfiguration(&dstorageConfig));

		CHK(DStorageGetFactory(IID_PPV_ARGS(&mDStorageFactory)));
		mDStorageFactory->SetDebugFlags(DSTORAGE_DEBUG_BREAK_ON_ERROR | DSTORAGE_DEBUG_SHOW_ERRORS);

		DSTORAGE_QUEUE_DESC dstorageQueueDesc = {};
		dstorageQueueDesc.Device = mDevice.Get();
		dstorageQueueDesc.Capacity = 1000;
		dstorageQueueDesc.Priority = DSTORAGE_PRIORITY_NORMAL;
		...
		CHK(mDStorageFile->GetFileInformation(&fileInfo));
		...
		for (int i = 0; i < MAX_DEFINED_RESOURCE; ++i)
		{
			...
			CHK(mDevice->CreateCommittedResource(
				&heapProp, D3D12_HEAP_FLAG_NONE, &resDesc,
				D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&mBindlessResource[i])));
			...
			req.UncompressedSize = sizeof(float[4]);
			req.Destination.Texture.Resource = mBindlessResource[i].Get();
			req.Destination.Texture.SubresourceIndex = 0;
			req.Destination.Texture.Region = CD3DX12_BOX(0, 1);
			req.CancellationTag = 1;
			static const char* debugNames[MAX_DEFINED_RESOURCE] = {
				"Tex0", "Tex1", "Tex2", "Tex3", "Tex4", "Tex5", "Tex6", "Tex7"
			};
			req.Name = debugNames[i];
			mDStorageQueue->EnqueueRequest(&req);
		}
		...
		// Submit DirectStorage

		CHK(mDStorageFence->SetEventOnCompletion(1, mDStorageSignalHandle));
		mDStorageQueue->EnqueueSignal(mDStorageFence.Get(), 1);
		mDStorageQueue->Submit();

		// Enqueue GPU wait

		CHK(mCmdQueue->Wait(mDStorageFence.Get(), 1));
	}

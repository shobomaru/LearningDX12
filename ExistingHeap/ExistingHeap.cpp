...
class D3D
{
	...
	ComPtr<ID3D12Device3> mDevice; // ID3D12Device3
	uint32_t mRTVStride;
	uint32_t mDSVStride;
	...
	ComPtr<ID3D12GraphicsCommandList2> mCmdList; // ID3D12GraphicsCommandList2
	...
	void* mExistingMemory = nullptr;
	D3D12_GPU_VIRTUAL_ADDRESS mExistingGpuMemory = {};
	ComPtr<ID3D12Heap> mExistingHeap;
	ComPtr<ID3D12Resource> mExistingResource;
...
public:
	~D3D()
	{
		...
		_snprintf_s(debugString, 128, "Last existing heap value is %d.\n", *reinterpret_cast<uint32_t*>(mExistingMemory));
		OutputDebugStringA(debugString);

		mExistingResource.Reset();
		mExistingHeap.Reset();
		VirtualFree(mExistingMemory, 0, MEM_RESERVE);
		mExistingMemory = nullptr;
	}
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		// Existing Heap
		mExistingMemory = VirtualAlloc(nullptr, 65536, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		CHK(mDevice->OpenExistingHeapFromAddress(mExistingMemory, IID_PPV_ARGS(&mExistingHeap)));
		resDesc = CD3DX12_RESOURCE_DESC::Buffer(65536, D3D12_RESOURCE_FLAG_ALLOW_CROSS_ADAPTER);
		CHK(mDevice->CreatePlacedResource(mExistingHeap.Get(), 0, &resDesc,
			D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&mExistingResource)));
		mExistingGpuMemory = mExistingResource->GetGPUVirtualAddress();
	}
	...
	void Draw()
	{
		...
		*reinterpret_cast<DirectX::XMMATRIX*>(pCBSceneMatrix) = DirectX::XMMatrixTranspose(worldMat * viewMat * projMat);
		...
		// Write a value at CPU visible memory
		D3D12_WRITEBUFFERIMMEDIATE_PARAMETER wbiParam = { mExistingGpuMemory, (UINT32)mFrameCount };
		D3D12_WRITEBUFFERIMMEDIATE_MODE wbiMode = D3D12_WRITEBUFFERIMMEDIATE_MODE_MARKER_OUT;
		mCmdList->WriteBufferImmediate(1, &wbiParam, &wbiMode);

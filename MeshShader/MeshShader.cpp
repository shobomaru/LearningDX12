...
public:
	...
	D3D(int width, int height, HWND hWnd)
	{
		...
		static const char shaderCodeSceneMS[] = R"#(
cbuffer CScene {
	float4x4 ViewProj;
};
struct Input {
...
uint FetchIndex(ByteAddressBuffer buf, uint id)
{
	uint d = buf.Load(4 * (id / 2));
	if (id & 1)
		d >>= 16;
	return (d & 0xFFFF);
}
Input FetchVertexData(ByteAddressBuffer buf, uint id)
{
	Input input;
	input.position = buf.Load3(24 * id);
	input.normal = buf.Load3(24 * id + 12);
	return input;
}
Output ConvertOutput(const Input input)
{
	Output output;
...
ByteAddressBuffer VBuf;
ByteAddressBuffer IBuf;
[NumThreads(64, 1, 1)]
[OutputTopology("triangle")]
void main(uint gtid : SV_GroupThreadID, uint dtid : SV_DispatchThreadID, out indices uint3 tris[64], out vertices Output verts[192]) {
	uint idx0 = FetchIndex(IBuf, 3 * dtid);
	uint idx1 = FetchIndex(IBuf, 3 * dtid + 1);
	uint idx2 = FetchIndex(IBuf, 3 * dtid + 2);
	Input vtx0 = FetchVertexData(VBuf, idx0);
	Input vtx1 = FetchVertexData(VBuf, idx1);
	Input vtx2 = FetchVertexData(VBuf, idx2);
	Output out0 = ConvertOutput(vtx0);
	Output out1 = ConvertOutput(vtx1);
	Output out2 = ConvertOutput(vtx2);
	SetMeshOutputCounts(128, 384);
	tris[3 * gtid] = 3 * gtid;
	tris[3 * gtid + 1] = 3 * gtid + 1;
	tris[3 * gtid + 2] = 3 * gtid + 2;
	verts[3 * gtid] = out0;
	verts[3 * gtid] = out1;
	verts[3 * gtid] = out2;
}
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		ComPtr<IDxcBlobEncoding> dxcTxtSceneMS, dxcTxtScenePS;
		CHK(dxcLib->CreateBlobWithEncodingFromPinned(shaderCodeSceneMS, _countof(shaderCodeSceneMS) - 1, CP_UTF8, &dxcTxtSceneMS));
		CHK(dxcLib->CreateBlobWithEncodingFromPinned(shaderCodeScenePS, _countof(shaderCodeScenePS) - 1, CP_UTF8, &dxcTxtScenePS));

		ComPtr<IDxcBlob> dxcBlobSceneMS, dxcBlobScenePS;
		...
		dxc->Compile(dxcTxtSceneMS.Get(), nullptr, L"main", L"ms_6_5", shaderArgs, _countof(shaderArgs), nullptr, 0, nullptr, &dxcRes);
		...
		dxcRes->GetResult(&dxcBlobSceneMS);
		...
		psoDesc.MS = CD3DX12_SHADER_BYTECODE(dxcBlobSceneMS->GetBufferPointer(), dxcBlobSceneMS->GetBufferSize());
		...
		CHK(mDevice.As(&device2));
		CHK(device2->CreatePipelineState(&streamDesc, IID_PPV_ARGS(&mScenePSO)));

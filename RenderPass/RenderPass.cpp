...
class D3D
{
	...
	ComPtr<ID3D12GraphicsCommandList4> mCmdList; // ID3D12GraphicsCommandList4
	...
	ComPtr<ID3D12Resource> mShadowZ;
...
public:
	...
	D3D(int width, int height, HWND hWnd)
	{
		...
		CHK(mDevice->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(), IID_PPV_ARGS(&mShadowRootSig)));

		descRange[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0); // VS
		descRange[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0); // PS
		descRange[2].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 2, 0); // PS
		descRange[3].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, 1, 0); // PS
		rootParam[0].InitAsDescriptorTable(1, descRange + 0, D3D12_SHADER_VISIBILITY_VERTEX); // CBV_SRV_UAV
		rootParam[1].InitAsDescriptorTable(2, descRange + 1, D3D12_SHADER_VISIBILITY_PIXEL); // CBV_SRV_UAV
		rootParam[2].InitAsDescriptorTable(1, descRange + 3, D3D12_SHADER_VISIBILITY_PIXEL); // Sampler
...
float4 main(float3 position : POSITION) : SV_Position {
	return mul(float4(position, 1), ShadowViewProj);
}
)#";
...
float4 main(Input input) : SV_Target {
	...
	return color;
		...
		CHK(mDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&mShadowPSO)));
		...
		CHK(mDevice->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &resDesc,
			D3D12_RESOURCE_STATE_DEPTH_WRITE, &clearValue, IID_PPV_ARGS(&mSceneZ))); // No transition used

		resDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, kShadowMapSize, kShadowMapSize, 1, 1);
		resDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
		clearValue = CD3DX12_CLEAR_VALUE(DXGI_FORMAT_D32_FLOAT, kDefaultDSClearColor);
		CHK(mDevice->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &resDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ, &clearValue, IID_PPV_ARGS(&mShadowZ)));
		...
		mDevice->CreateDepthStencilView(mShadowZ.Get(), nullptr, dsvHandle);
		...
		for (int i = 0; i < BUFFER_COUNT; i++)
		{
			...
			auto sv = CD3DX12_CPU_DESCRIPTOR_HANDLE(shaderViewHandle, (int)ShaderViews::ShadowSRV, mResourceStride);
			mDevice->CreateShaderResourceView(mShadowZ.Get(), &srv, sv);

			D3D12_CONSTANT_BUFFER_VIEW_DESC cbv = {};
			cbv.BufferLocation = addrCB + 256 * (int)Constants::ShadowMatrix;
			cbv.SizeInBytes = 256;
			sv = CD3DX12_CPU_DESCRIPTOR_HANDLE(shaderViewHandle, (int)ShaderViews::ShadowCBVMatrix, mResourceStride);
			mDevice->CreateConstantBufferView(&cbv, sv);
			sv = CD3DX12_CPU_DESCRIPTOR_HANDLE(shaderViewHandle, (int)ShaderViews::ScenePSCBVMatrix, mResourceStride);
			mDevice->CreateConstantBufferView(&cbv, sv);

			cbv.BufferLocation = addrCB + 256 * (int)Constants::SceneMatrix;
			cbv.SizeInBytes = 256;
			sv = CD3DX12_CPU_DESCRIPTOR_HANDLE(shaderViewHandle, (int)ShaderViews::SceneCBVMatrix, mResourceStride);
			mDevice->CreateConstantBufferView(&cbv, sv);
		}
	...
	void Draw()
	{
		...
		float* pCBShadowMatrix = pCB + 256 * (int)Constants::ShadowMatrix / sizeof(*pCB);
		...
		auto svScenePS = CD3DX12_GPU_DESCRIPTOR_HANDLE(svBase, (int)ShaderViews::ShadowSRV, mResourceStride);

		auto samplerShadow = CD3DX12_GPU_DESCRIPTOR_HANDLE(mSampler->GetGPUDescriptorHandleForHeapStart());
		...
		*reinterpret_cast<DirectX::XMMATRIX*>(pCBShadowMatrix) = DirectX::XMMatrixTranspose(shadowViewMat * shadowProjMat);
		...
		transitions[0] = CD3DX12_RESOURCE_BARRIER::Transition(mShadowZ.Get(),
			D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_DEPTH_WRITE);
		mCmdList->ResourceBarrier(1, transitions);

		D3D12_RENDER_PASS_RENDER_TARGET_DESC renderPassRT = {}; // Shadow pass does not use render targets.
		D3D12_RENDER_PASS_DEPTH_STENCIL_DESC renderPassDS = {};
		renderPassDS.cpuDescriptor = dsvShadow;
		...
		mCmdList->SetGraphicsRootDescriptorTable(0, svShadow); // VS, CBV_SRV_UAV
		mCmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		mCmdList->IASetVertexBuffers(0, 1, &mVBView);
		mCmdList->IASetIndexBuffer(&mIBView);
		mCmdList->RSSetViewports(1, &CD3DX12_VIEWPORT(0.0f, 0.0f, (float)kShadowMapSize, (float)kShadowMapSize));
		mCmdList->RSSetScissorRects(1, &CD3DX12_RECT(0, 0, kShadowMapSize, kShadowMapSize));
		//mCmdList->OMSetRenderTargets(0, nullptr, TRUE, &dsvShadow);
		...
		transitions[1] = CD3DX12_RESOURCE_BARRIER::Transition(mSceneTex.Get(),
			D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET);
		mCmdList->ResourceBarrier(2, transitions);
		
		renderPassRT = {};
		renderPassRT.cpuDescriptor = rtvScene;
		renderPassRT.BeginningAccess.Type = D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR;
		renderPassRT.BeginningAccess.Clear.ClearValue = CD3DX12_CLEAR_VALUE(DXGI_FORMAT_R8G8B8A8_UNORM, kDefaultRTClearColor);
		renderPassRT.EndingAccess.Type = D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_PRESERVE;
		renderPassDS = {};
		renderPassDS.cpuDescriptor = dsvScene;
		renderPassDS.DepthBeginningAccess.Type = D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR;
		renderPassDS.DepthBeginningAccess.Clear.ClearValue = CD3DX12_CLEAR_VALUE(DXGI_FORMAT_D32_FLOAT, kDefaultDSClearColor[0], 0);
		...
		mCmdList->SetGraphicsRootDescriptorTable(2, samplerShadow); // PS, Sampler
		mCmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		mCmdList->IASetVertexBuffers(0, 1, &mVBView);
		mCmdList->IASetIndexBuffer(&mIBView);
		mCmdList->RSSetViewports(1, &CD3DX12_VIEWPORT(0.0f, 0.0f, (float)WINDOW_WIDTH, (float)WINDOW_HEIGHT));
